...
export default function BenefitsBar() {
  ...
  return (
    <div id="benefits" className="border-y border-white/[0.05] bg-brand-black3 py-[18px]">
      <div className="mx-auto flex max-w-site flex-wrap justify-center gap-12 px-7">
        {items.map((item) => (
          <div key={item.title} className="flex items-center gap-3">
            ...
            <div>
              <strong className="block text-[13px] font-extrabold text-brand-cream">
                {item.title}
              </strong>
              <span className="text-[11px] uppercase tracking-wide text-brand-cream/40">
                {item.sub}
              </span>
            </div>
          </div>
